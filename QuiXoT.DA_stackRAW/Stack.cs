...
namespace QuiXoT.DA_stackRAW
{
    ...
    public class binStack
    {
        ...
        */
        ...
        public static int countFramesFromIndex(binStack[] _stackIndex)
        {
            int numOfFrames=0;

            //get the maximum number of the frame
            for (int i = _stackIndex.GetLowerBound(0); i <= _stackIndex.GetUpperBound(0); i++)
            {
                for (int j = _stackIndex[i].scan.GetLowerBound(0); j <= _stackIndex[i].scan.GetUpperBound(0); j++)
                {
                    if (_stackIndex[i].scan[j].frame > numOfFrames) numOfFrames = _stackIndex[i].scan[j].frame;
                }
            }
